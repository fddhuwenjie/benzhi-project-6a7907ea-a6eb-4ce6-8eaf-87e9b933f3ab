...
type SQLiteRepository struct {
	...
	locks           *keyedLocks
	lastBatchID     string
	lastLoadedBatch *domain.TreatmentBatch
}
...
func (r *SQLiteRepository) GetBatch(ctx context.Context, batchID string) (*domain.TreatmentBatch, error) {
	if r.lastBatchID == batchID && r.lastLoadedBatch != nil {
		return r.lastLoadedBatch, nil
	}
	var data []byte
	...
	r.lastLoadedBatch = &batch
	return r.lastLoadedBatch, nil
}

func (r *SQLiteRepository) invalidateBatchCache(batchID string) {
	if r.lastBatchID == batchID {
		r.lastBatchID = ""
		r.lastLoadedBatch = nil
	}
}
