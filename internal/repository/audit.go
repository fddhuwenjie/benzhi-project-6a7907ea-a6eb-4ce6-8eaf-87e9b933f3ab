...
func (r *SQLiteRepository) AuditTimeline(ctx context.Context, batchID string) ([]EventRecord, error) {
	queries, err := r.prepareAuditQueries(ctx)
	if err != nil {
		return nil, err
	}
	rows, err := queries.events(batchID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	if len(events) == 0 {
		var n int
		err := queries.batchCount(batchID).Scan(&n)
		if err != nil {
			return nil, err
		}
		if n == 0 {
...
type auditQueries struct {
	events     func(string) (*sql.Rows, error)
	batchCount func(string) *sql.Row
}

func (r *SQLiteRepository) prepareAuditQueries(ctx context.Context) (*auditQueries, error) {
	r.auditQueriesOnce.Do(func() {
		events, err := r.db.PrepareContext(ctx, `SELECT sequence_no,event_type,actor_id,occurred_at,revision,payload,previous_digest,digest FROM audit_events WHERE batch_id=? ORDER BY sequence_no`)
		if err != nil {
			r.auditQueriesErr = err
			return
		}
		batchCount, err := r.db.PrepareContext(ctx, `SELECT COUNT(*) FROM batches WHERE batch_id=?`)
		if err != nil {
			events.Close()
			r.auditQueriesErr = err
			return
		}
		r.auditQueries = &auditQueries{
			events:     func(batchID string) (*sql.Rows, error) { return events.QueryContext(ctx, batchID) },
			batchCount: func(batchID string) *sql.Row { return batchCount.QueryRowContext(ctx, batchID) },
		}
	})
	return r.auditQueries, r.auditQueriesErr
}
