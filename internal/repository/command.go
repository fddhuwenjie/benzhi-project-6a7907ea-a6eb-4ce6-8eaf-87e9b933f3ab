...
func (r *SQLiteRepository) execute(ctx context.Context, batchID, requestID, fingerprint string, operation func(*sql.Tx) ([]byte, error)) (CommandResult, error) {
	...
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			body = []byte("null")
			_, storeErr := tx.ExecContext(ctx, `INSERT INTO idempotency(request_id,batch_id,fingerprint,response_body,committed_at) VALUES(?,?,?,?,?)`, requestID, batchID, fingerprint, body, time.Now().UTC().Format(time.RFC3339Nano))
			if storeErr != nil {
				return CommandResult{}, storeErr
			}
			if commitErr := tx.Commit(); commitErr != nil {
				return CommandResult{}, commitErr
			}
		}
		return CommandResult{}, err
	}
